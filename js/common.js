    ...
    $.ajax({
        ...
        success: function(response){
            ...
            if (response.vote) {
                ...
                $('.vote[data-id=' + itemId + ']').text(response.vote);

            } else {

                $('.item-like-button[data-id=' + itemId + ']').removeClass("active");
            }
            console.log(response.mylike); 
            if(response.mylike == false){
                $('.item-like-button[data-id=' + itemId + ']').removeClass("active");
            }else{
                $('.item-like-button[data-id=' + itemId + ']').addClass("active");
            }
...
Profile.edit = function (username) {
    ...
    $.ajax({
        ...
        timeout: 30000,
        success: function(response) {
            window.location.href="http://barugasikola.com/account/wall";
            // $('textarea[name=postText]').val('');
            // $("div.img_container").hide();
            // $('#word_counter').html('1000');
...
Profile.lomba = function (username) {
    ...
    // console.log($("div.img-items-list-page").length);
    // if($("div.img-items-list-page").length == 0){
    //     console.log("BBBB");
    //     return false;
    // }
    $.ajax({
...
Profile.addPostVideo = function(url,type) {
    ...
    $('div.img-items-list-page').append('<div class="gallery-item new-post-img-item">\n' +
        ...
        '\n' +
        '                               <video width="100%" height="150px" id="player">\n'+
        '\n'+
        '                                <source src="'+url+'" type="video/'+type+'" controls="">\n'+
        '\n'+
...
Comments.reply = function(replyToUserId, replyToUserUsername, replyToUserFullname, commentReplyId) {
    console.log("aaaa");
    window.replyToUserId = replyToUserId;
    if(commentReplyId != ""){
    $("#comment_reply_id").val(commentReplyId);
    }
    console.log("@" + replyToUserUsername + ", ");
    $("textarea[name=comment_text]").val("@" + replyToUserUsername + ", ");
    $("textarea[name=comment_text]").focus();
}

Comments.create = function (itemId, itemType) {
    console.log("AAAAAbbbb");
    var commentText = $("textarea[data-id=" + itemId + "]").val();
    var commentReplyId = "0";
    var replyToUserId = window.replyToUserId;
    if(replyToUserId != 0){
        commentReplyId = $("#comment_reply_id").val();
    }
    $.ajax({
        type: 'POST',
        url: '/ajax/comments/new/',
        data: 'accountId=' + account.id + '&accessToken=' + account.accessToken + '&itemId=' + itemId + '&itemType=' + itemType + '&commentText=' + commentText + '&replyToUserId=0'+replyToUserId+'commentReplyId='+commentReplyId,
        dataType: 'json',
        timeout: 30000,
        success: function(response){
            ...
            if (response.hasOwnProperty('html')) {
                ...
                $('.comments-list').append(response.html);
                    ...
                    console.log("input[data-id=comment-" + commentReplyId + "]"); 
                    $(".commentall[data-id=comment-" + commentReplyId + "]").append(response.html);
                }

                //$(response.html).insertBefore(".comment_form[data-id=" + postId + "]");
                $("textarea[data-id=" + itemId + "]").val("");
                replyToUserId = 0;
            }else{
                console.log("C");
            }
